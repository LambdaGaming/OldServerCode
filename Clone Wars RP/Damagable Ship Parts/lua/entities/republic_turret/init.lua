...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create( "republic_turret" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	return ent
...
function ENT:Use(caller, activator)
	caller:ChatPrint("Turret mainframe health: "..( math.Round( self:Health(), 1 ) )..".")
end
...
function ENT:OnTakeDamage()
	...
	if self:Health() <= 0 then
		...
		for k,v in pairs(player.GetAll()) do
			v:ChatPrint("The Republic turret mainframe has been destroyed!")
		end
	end
end
